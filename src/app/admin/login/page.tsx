...
import { Lock, User, KeyRound, AlertCircle, Loader2, ArrowLeft, Clock } from 'lucide-react';
...
function AdminLoginForm() {
  ...
  const isInactiveExpired = reason === 'inactivity' || reason === 'expired';
  ...
  return (
    <div className="w-full max-w-md space-y-6 relative z-10">
      ...
      <div className="bg-slate-900/90 border border-slate-800 backdrop-blur-xl rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        {isInactiveExpired && !errorMessage && (
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-start gap-2.5 shadow-sm">
            <Clock size={18} className="shrink-0 mt-0.5 text-amber-400" />
            <span>
              Sesi Anda telah diakhiri secara otomatis demi keamanan karena tidak ada aktivitas selama 5 menit. Silakan login kembali.
            </span>
          </div>
        )}
