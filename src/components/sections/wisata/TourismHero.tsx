...
import { ChevronRight, Palmtree, Compass } from 'lucide-react';

interface TourismHeroProps {
  totalSpots?: number;
}

export default function TourismHero({ totalSpots = 4 }: TourismHeroProps) {
  return (
    <section className="relative bg-gradient-to-br from-slate-900 via-slate-850 to-primary-950 text-white py-16 md:py-24 pt-36 sm:pt-40 md:pt-36 overflow-hidden">
      ...
      <div className="container-section relative z-10 text-center max-w-3xl mx-auto space-y-4">
        ...
        <nav className="flex items-center justify-center gap-2 text-xs text-slate-400 font-medium">
          ...
          <span className="text-primary-400 font-semibold">Wisata & Keindahan Alam</span>
        </nav>
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
          <Palmtree size={14} className="text-emerald-400 animate-pulse" />
          <span className="text-xs text-white/80 font-medium">
            Destinasi Alam & Saung Santai Keluarga
          </span>
        </div>
        ...
        <p className="text-slate-300 text-sm sm:text-base md:text-lg leading-relaxed text-balance">
          Nikmati pemandangan hijau perbukitan Sidomulyo, saung kumpul di tengah sawah, serta aliran sungai jernih yang asri dan menenangkan.
        </p>

        {/* Count Pill */}
        <div className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
          <Compass size={13} />
          <span>{totalSpots} Destinasi Alam Terdaftar</span>
        </div>
      </div>
    </section>
  );
