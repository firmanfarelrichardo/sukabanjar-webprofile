...
import { Target, Compass, Plus, Trash2 } from 'lucide-react';
...
export default function VisionMissionSection({ vision, missions }: VisionMissionProps) {
  ...
  return (
    <section className="py-16 md:py-24 bg-[#f8fafc] relative overflow-hidden border-b border-slate-200/80">
      <div className="container-section">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Left Column: Visi Utama Light Theme Card */}
          <div className="lg:col-span-5 flex">
            <div className="w-full rounded-3xl bg-white border border-slate-200 shadow-xl text-slate-900 p-8 sm:p-10 flex flex-col justify-between relative overflow-hidden hover:border-[#0086C9]/40 transition-all duration-300">
              <div className="space-y-6 relative z-10">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-extrabold px-3 py-1 rounded-full bg-[#0086C9] text-white shadow-sm">
                    VISI DESA
                  </span>
                </div>

                <div className="space-y-3">
                  <h3 className="text-xs uppercase font-extrabold text-[#0086C9] tracking-wider">
                    Cita-Cita Utama
                  </h3>
                  ...
                  {isAdmin && isEditMode ? (
                    <div className="space-y-1">
                      <label className="block text-[11px] font-bold text-amber-600 uppercase">
                        Edit Visi Utama Langsung:
                      </label>
                      <textarea
                        rows={4}
                        value={visionText}
                        onChange={(e) => handleVisionChange(e.target.value)}
                        className="w-full text-lg font-bold font-heading leading-snug text-slate-900 bg-amber-50/50 p-3 rounded-2xl border-2 border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-500"
                        placeholder="Masukkan visi utama desa..."
                      />
                    </div>
                  ) : (
                    <blockquote className="text-xl sm:text-2xl font-extrabold font-heading leading-snug text-slate-900 italic">
                      "{visionText}"
                    </blockquote>
                  )}
                </div>
              ...
              <div className="pt-8 relative z-10 border-t border-slate-100 mt-6 flex items-center gap-2 text-xs text-slate-600 font-semibold">
                <Compass size={16} className="text-[#0086C9] shrink-0" />
          ...
          <div className="lg:col-span-7 flex">
            <div className="w-full rounded-3xl bg-white border border-slate-200/80 p-7 sm:p-9 shadow-md flex flex-col justify-between space-y-6">
              <div className="space-y-6">
                <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold px-3.5 py-1 rounded-full bg-[#0086C9]/10 text-[#0086C9] border border-[#0086C9]/20">
                ...
                <ul className="space-y-4">
                  {missionList.map((item, index) => (
                    <li key={index} className="flex items-start gap-4 group">
                      <div className="w-8 h-8 rounded-xl bg-[#0086C9]/10 text-[#0086C9] border border-[#0086C9]/20 flex items-center justify-center shrink-0 mt-0.5 font-extrabold text-xs">
                        {index + 1}
                      </div>
