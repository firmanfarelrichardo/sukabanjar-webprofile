...
export default function MapHero({ totalLocations = 6 }: MapHeroProps) {
  return (
    <section className="relative bg-gradient-to-br from-slate-900 via-slate-850 to-primary-950 text-white py-16 md:py-24 pt-36 sm:pt-40 md:pt-36 overflow-hidden">
      ...
      <div className="container-section relative z-10 text-center max-w-3xl mx-auto space-y-4">
        ...
        <nav className="flex items-center justify-center gap-2 text-xs text-slate-400 font-medium">
          ...
          <span className="text-primary-400 font-semibold">Peta Fasilitas</span>
        </nav>
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
          <Navigation size={14} className="text-accent-400 animate-pulse" />
          <span className="text-xs text-white/80 font-medium">
            Pemetaan Geospasial Desa
          </span>
        </div>
        ...
        <p className="text-slate-300 text-sm sm:text-base md:text-lg leading-relaxed text-balance">
          Temukan sebaran lokasi kantor pemerintahan balai desa, sekolah, sarana kesehatan, tempat ibadah, serta titik UMKM & potensi desa secara presisi.
        </p>

        {/* Total Locations Pill */}
        <div className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full bg-primary-500/20 text-primary-300 border border-primary-500/30">
          <MapPin size={13} />
          <span>{totalLocations} Titik Lokasi Terdaftar</span>
        </div>
      </div>
    </section>
  );
